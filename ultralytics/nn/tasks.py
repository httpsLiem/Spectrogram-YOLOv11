    ...
    def fuse(self, verbose=True):
        ...
        if not self.is_fused():
            for m in self.model.modules():
                if isinstance(m, (Conv, RecycleConv, Conv2, DWConv, DDWConv,ConvOMN, MaxConv, ConvHCA, GhostMaxConv, GhostConvMax,GhostConvHCA, MaxConvHCA, FilterMaxConv)) and hasattr(m, "bn"):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            if m is C3k2:  # for M/L/X sizes
                ...
                if scale in "mlx":
                    args[3] = True
            if m is C3MSCk2:  # for M/L/X sizes
                legacy = False
                if scale in "mlx":
                    args[3] = True
            if m is XCBAM2C2f:  # for M/L/X sizes
                legacy = False
                if scale in "mlx":
                    args[3] = True
            if m is C3k2GC:  # for M/L/X sizes
                legacy = False
                if scale in "mlx":
                    args[3] = True
        elif m is AIFI:
